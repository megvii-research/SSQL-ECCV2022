...
from quant_tools.blocks import QuantConv2d, QuantLinear, QuantBasic, NAME_QBLOCK_MAPPING
from quant_tools import QuantModel


def get_data_samples(data_loader, num_samples):
    train_data = []
    ...
    return train_data[: min(num_samples, len(train_data))]
    ...
    def __call__(self, model_input):
        ...
        with torch.no_grad():
            ...
            if self.asym:
                ...
                self.model.set_quant_state(w_quant=True, a_quant=self.act_quant)
                try:
                    _ = self.model(model_input.to(self.device))
                except StopForwardException:
                    pass
...
def quantize_model_till(model: QuantModel, layer: QuantBasic, act_quant: bool = False):
    """
    ...
    for name, module in model.named_modules():
        if isinstance(module, tuple(BLOCK_NAME_MAPPING.keys())):
